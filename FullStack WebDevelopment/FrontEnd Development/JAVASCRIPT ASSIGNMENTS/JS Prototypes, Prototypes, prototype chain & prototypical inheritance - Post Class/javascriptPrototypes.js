...
Array.prototype.includesOneof = function (values) {
    const arr = this;
    ...
    for (let i = 0; i < values.length; i++) {
        if(values[i] instanceof Object){
            ...
            const stringifiedValue = JSON.stringify(values[i]);
            const match = arr.find(item => JSON.stringify(item) === stringifiedValue)
